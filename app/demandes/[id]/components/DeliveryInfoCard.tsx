    ...
    <Card>
      ...
      <CardContent className="space-y-4">
        <div>
          ...
          {modeEdition ? (
            <Textarea {...register("delivery.address")} rows={2} className={`mt-1 ${validationErrors.delivery?.address ? "border-red-500" : ""}`} />
          ) : (
            <p className="mt-1">{demande.delivery.address}</p>
          )}
          {validationErrors.delivery?.address && <p className="text-red-500 text-sm mt-1">{validationErrors.delivery.address.message}</p>}
        </div>
        ...
        <div>
          ...
          {validationErrors.delivery?.tel && <p className="text-red-500 text-sm mt-1">{validationErrors.delivery.tel.message}</p>}
        </div>

        <div>
          <Label className="text-sm font-medium">Commentaire</Label>
          {modeEdition ? (
            <Textarea {...register("delivery.comment")} rows={2} className={`mt-1 ${validationErrors.delivery?.comment ? "border-red-500" : ""}`} />
          ) : (
            <p className="mt-1">{demande.delivery.comment || "Non spécifié"}</p>
          )}
          {validationErrors.delivery?.comment && <p className="text-red-500 text-sm mt-1">{validationErrors.delivery.comment.message}</p>}
        </div>
      </CardContent>
    </Card>
  );
