...
export default function DetailDemande() {
  ...
  const orderTotal = watchedItems.reduce((acc: number, item: { price?: number; quantity?: number; unitPrice?: number }) => {
    const linePrice = typeof item?.price === "number" ? item.price : (item?.quantity ?? 0) * (item?.unitPrice ?? 0);
  ...
  const gererSauvegarde = (data: DemandeFormData, files?: File[]) => {
    ...
    updateDemandeWithJsonFileMutation.mutate(
      { requests: payload, files: files || [] },
      {
        onSuccess: () => {
  ...
  const handleValidateAndSubmit = () => {
    ...
    console.log("Demande ready for submission:", demandeUpdated);
  ...
  const handleConfirmDeleteFile = () => {
    if (fichierASupprimer) {
      ...
      const deletedWasInvoice = currentFiles.some(
        (f: FileType) => (f.id === fichierASupprimer.id || f.name === fichierASupprimer.name) && f.category === "invoices"
      );
  ...
  if (error) {
    ...
    return (
      <div className="min-h-screen bg-gray-50 flex items-center justify-center">
        <div className="text-center">
          <h2 className="text-2xl font-bold text-gray-900 mb-2">{isNotFound ? "Demande non trouvée" : "Erreur"}</h2>
          <p className="text-gray-600 mb-4">
            {isNotFound ? `La demande avec l'ID ${params.id} n'existe pas.` : `Impossible de charger la demande : ${error.message}`}
          </p>
  ...
  return (
    <div className="min-h-screen bg-gray-50">
        ...
        <div className="space-y-6 px-6">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div className="space-y-6">
              <SupplierInfoCard demande={demande} modeEdition={modeEdition} validationErrors={modeEdition ? errors : {}} register={register} />
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            <BillingInfoCard demande={demande} modeEdition={modeEdition} validationErrors={modeEdition ? errors : {}} register={register} />

            <DeliveryInfoCard demande={demande} modeEdition={modeEdition} validationErrors={modeEdition ? errors : {}} register={register} />
      ...
      {/* Modal PDF */}
      <PDFModal isOpen={showPDFModal} onClose={() => setShowPDFModal(false)} demande={demande} />
